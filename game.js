//geting elements from game.html
...
fetch("https://opentdb.com/api.php?amount=10&category=9&difficulty=easy&type=multiple")
  .then((response) => {   // receive data from the api
    ...
    questions = loadingQuestions.results.map((loadingQuestion) => {
      ...
      answerChoices.splice(formattedQuestion.question - 1, 0, loadingQuestion.correct_answer);
...
getNewQuestion = () => {
  //to check if there's no more question in the available array or if questionCounter(0) is equeal to 7 
  if (availableQuestion.length === 0 || questionCounter >= MAX_QUESTION) {
    localStorage.setItem("mostRecentScore", score);


    //to take us to the end page
...
choices.forEach((choice) => {
  choice.addEventListener("click", (e) => {
    ...
    const selectedAnswer = selectedc = choice.dataset["number"];
